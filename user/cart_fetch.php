<?php
	...
	if(isset($_SESSION['user'])){
		try{
			...
			foreach($stmt as $row){
				$output['count']++;
				$image = (!empty($row['photo'])) ? '../database/images/'.$row['photo'] : '../database/images/noimage.jpg';
		...
		if(empty($_SESSION['cart'])){
			...
			foreach($_SESSION['cart'] as $row){
				...
				$product = $stmt->fetch();
				$image = (!empty($product['photo'])) ? '../database/images/'.$product['photo'] : '../database/images/noimage.jpg';
				$output['list'] .= "
					<li>
